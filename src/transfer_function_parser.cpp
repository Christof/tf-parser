...
public:
    ...
    bool fatalError(const QXmlParseException & exception)
    {
        std::cout <<
                     "Fatal error on line"  <<
                     exception.message().toStdString() << std::endl;
    ...
    void parseGradientAttributes(const QXmlAttributes & atts)
    {
        ...
        instance->setCoordinateMode(parseCoordinateMode(atts.value("coordinateMode")));
    ...
    {
        std::cout << "Read End Element Tag : " << localName.toStdString()<< std::endl;
        if (localName.compare("gradientData") == 0){
